...
class MainWin(QWidget):
    def __init__(self):
        ...
        self.btn_video = QPushButton("Streaming")
        self.btn_close = QPushButton("Stop video")
        ...
        self.btn_close.clicked.connect(self.stop_video)
    ...
    def set_image(self):
        img = QImage("img/camera.png")
        img = img.scaled(640, 480, Qt.KeepAspectRatio)
        self.lb_camera_runtime.setPixmap(QPixmap.fromImage(img))
        self.lb_camera_detection.setPixmap(QPixmap.fromImage(img))
        ...
        while self.thread_on:
            ...
            if ret:
               ...
               lower_yellow, upper_yellow = get_limits("yellow")
                ...
                lower_red, upper_red = get_limits("red2")
                mask_red = cv2.inRange(
                    hsv_img, np.array(lower_red), np.array(upper_red)
                )
                bbox_red = Image.fromarray(mask_red).getbbox()
