...
#define CODE_CURRENT_MESSAGE      'M'                       // Codigo ASCII para obter a mensagem atual traduzida
...
String data_backup = "ola";                                 // Copia que armazena a mensagem recebida
...
void execute_translation(void) {
  ...
  for (size_t i = 0; i < data_backup.length(); i++) {
    define_caractere(data_backup[i]);
    exibe_caractere();
...
void obtain_message(void) {
  comunication_mode("SEND");
  Serial.print("Current message translated: ");
  Serial.println(data_backup);
  delay(200);
  comunication_mode("RECEIVE");
}
