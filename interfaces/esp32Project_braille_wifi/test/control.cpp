 ...
 */
#define CODE_TRANSLATE_MESSAGE                                                 \
  'T' // Código ASCII para traduzir a mensagem recebida
#define CODE_EXECUTE_MESSAGE                                                   \
  'E' // Código ASCII para executar a mensagem recebida
#define CODE_CHANGE_SPEED                                                      \
  'C' // Código ASCII para alterar a velocidade de execução da mensagem
...
void comunication_mode(
    String mode);           // Função para alternar entre modos de comunicação
...
void setup() {
  ...
  exibe_caractere(); // Inicializa os pinos em estado baixo (nenhum ponto
                     // ativado)
...
void checkSerial(void) {
  if (Serial.available()) { // Se houver dados disponíveis na serial
    ...
    if (frame_received.length() > 0) { // Se o quadro tiver dados
      ...
      String lengthOfData =
          frame_received.substring(2, 4); // Extrai o comprimento dos dados
      data_length = lengthOfData.toInt(); // Converte o comprimento para inteiro
      data_received =
          frame_received.substring(4, 4 + data_length); // Extrai os dados
      String crcReceived =
          frame_received.substring(4 + data_length); // Extrai o CRC recebido
      crc_received = crcReceived.toInt(); // Converte o CRC para inteiro

      // Calcula o CRC da mensagem recebida
      String crcMessage =
          address_received + function_received + lengthOfData + data_received;
      unsigned short crc_calculated =
          crc.XModemCrc((byte *)crcMessage.c_str(), 0, crcMessage.length());

      // Verifica se o CRC calculado é igual ao recebido
      crc_check = (crc_calculated == crc_received);
      enable_program = address_received.equals(
          String(ADDRESS)); // Habilita o programa se o endereço for válido
    }
  }
}
...
void setPinout(void) {
  ...
  pinMode(SR_COM,
          OUTPUT); // Configura o pino de controle de comunicação como saída
  // pinMode(TX_1, OUTPUT);                                  // Configura o pino
  // de controle de comunicação como saída pinMode(RX_1, OUTPUT); // Configura o
  // pino de controle de comunicação como saída
...
void execute_program(void) {
  ...
  if (enable_program) {          // Se o programa está habilitado para execução
    ...
    comunication_mode("SEND");            // Alterna para modo de envio
    Serial.println("ADDRESS NOT FOUND!"); // Envia mensagem de erro caso o
                                          // endereço não seja encontrado
    delay(200);
    comunication_mode("RECEIVE"); // Retorna ao modo de recepção
  }
}
...
void compile_translation(void) {
  if (data_length > 0) {
    ...
    Serial.print("Executing translation..."); // Exibe mensagem indicando
    ...
    data_backup =
        data_received; // Armazena a mensagem recebida para futuras execuções
  } else {             // Caso não tenha mensagem a ser traduzida
    comunication_mode("SEND"); // Configura para enviar dados pela serial
    Serial.println(
        "No message found to translation..."); // Exibe mensagem de erro
    delay(200);                   // Pequena pausa para garantir sincronização
    comunication_mode("RECEIVE"); // Volta a receber dados pela serial
...
void execute_translation(void) {
  comunication_mode("SEND"); // Configura para enviar dados pela serial
  Serial.println("Executing program :)"); // Exibe mensagem indicando início da
                                          // execução do programa
  delay(200);                   // Pequena pausa para garantir sincronização
  comunication_mode("RECEIVE"); // Volta a receber dados pela serial
  reset_output();    // Reseta os pinos de controle (desativa todos os pontos do
                     // caractere)
  ...
  for (size_t i = 0; i < data_backup.length(); i++) {
    define_caractere(
        data_backup[i]); // Define o padrão dos pontos para o caractere atual
...
void change_speed(void) {
  comunication_mode("SEND"); // Configura para enviar dados pela serial
  Serial.print(
      "Change speed ... "); // Exibe mensagem indicando alteração da velocidade
  ...
  comunication_mode("RECEIVE");  // Volta a receber dados pela serial
  time_slap =
      data_received.toInt(); // Converte e armazena o novo tempo de intervalo
  delay(1000);               // Pausa antes de executar o próximo bloco

  // Faz o LED piscar 6 vezes para indicar a mudança de velocidade
  for (size_t i = 0; i < 6; i++) {
    digitalWrite(LED_BOARD,
                 !digitalRead(LED_BOARD)); // Alterna o estado do LED (pisca)
...
void obtain_speed(void) {
  comunication_mode("SEND"); // Configura para enviar dados pela serial
  Serial.print(
      "Getting current speed: "); // Exibe mensagem de obtenção da velocidade
...
void obtain_message(void) {
  comunication_mode("SEND"); // Configura para enviar dados pela serial
  Serial.print("Current message translated: "); // Exibe mensagem indicando a
                                                // mensagem atual traduzida
